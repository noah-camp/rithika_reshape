function varargout = blockEdfLoadAnno(varargin)
...
if nargout >=3
    ...
    try
        ...
        if nargin == 1
            ...
            % Load annotations
            origin = ftell(fid);
            fseek(fid, signalHeader(1).samples_in_record*(num_signals-1)*2, 0);
            A = zeros(signalHeader(end).samples_in_record*2*header.num_data_records,1);
            for i=1:header.num_data_records
                A(((i-1)*signalHeader(end).samples_in_record*2)+1:i*signalHeader(end).samples_in_record*2) = fread(fid, signalHeader(end).samples_in_record*2, 'int8');
                fseek(fid, signalHeader(1).samples_in_record*(num_signals-1)*2, 0);
            end
            ...
            A = strsplit(A, char(20));
            annotations = string(zeros(length(A)*2, 2));
            
            for i=1:(length(A)-1) % May have to adjust this -1 later?
                if strcmp(A{i}(1), '+')
                    pos = i;
                    time = A{i};
                    annotations(i, 1) = time;
                    annotations(i, 2) = '';
                else
                    annotations(pos, 1) = time;
                    annotations(pos, 2) = A{i};
                    pos = pos+1;
                end
            end
            
            % Remove zeros, should optimize...
            pos = 1;
            for i=1:(length(annotations))
                if(strcmp(annotations(pos, 1), '0'))
                    annotations(pos, :) = [];
                elseif(strcmp(annotations(pos, 2), '0'))
                    annotations(pos, :) = [];
                else
                    pos = pos+1;
                end
